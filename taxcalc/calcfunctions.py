...
def EI_PayrollTax(SS_Earnings_c, e00200p, e00200s, pencon_p, pencon_s,
                  ...
                  sey, earned, earned_p, earned_s, FICA_ss_em):
    """
    Compute part of total OASDI+HI payroll taxes and earned income variables.
    """
    # compute sey and its individual components
    sey_p = max(e00900p + e02100p + k1bx14p - FICA_ss_em, 0)
    sey_s = max(e00900s + e02100s + k1bx14s - FICA_ss_em, 0)
    sey = sey_p + sey_s  # total self-employment income for filing unit

    # compute sey without exemption for medicare tax
    sey_p_mc = e00900p + e02100p + k1bx14p
    sey_s_mc = e00900s + e02100s + k1bx14s

    # compute gross wage and salary income ('was' denotes 'wage and salary')
    gross_was_p = max(e00200p + pencon_p - FICA_ss_em, 0)
    gross_was_s = max(e00200s + pencon_s - FICA_ss_em, 0)

    # compute gross wage adn salary income without exemption for medicare tax
    gross_was_p_mc = e00200p + pencon_p
    gross_was_s_mc = e00200s + pencon_s
    ...
    ptax_mc_was_p = FICA_mc_trt * gross_was_p_mc
    ptax_mc_was_s = FICA_mc_trt * gross_was_s_mc
    ...
    setax_mc_p = FICA_mc_trt * max(0., sey_p_mc * sey_frac)
    setax_mc_s = FICA_mc_trt * max(0., sey_s_mc * sey_frac)
